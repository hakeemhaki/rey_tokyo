...
if(!class_exists('ReyCore_Widget_Carousel_Uno')):
	...
	class ReyCore_Widget_Carousel_Uno extends \Elementor\Widget_Base {
		...
		public function rey_get_script_depends() {
			return [ 'rey-videos', 'rey-splide', 'imagesloaded', 'reycore-widget-carousel-uno-scripts' ];
		}
		...
		protected function register_controls() {
			...
			$this->add_control(
				'stretch_image',
				[
					'label' => esc_html__( 'Fully stretch image', 'rey-core' ),
					'type' => \Elementor\Controls_Manager::SWITCHER,
					'default' => '',
					'prefix_class' => 'cUno-imgstretch-',
				]
			);
			...
			$this->add_responsive_control(
				...
				[
				   'label' => esc_html__( 'Nav Spacing', 'rey-core' ),
					'type' => \Elementor\Controls_Manager::SLIDER,
					'range' => [
						'px' => [
							'min' => 0,
		...
		public function render_start(){
			...
			if( count($this->_items) > 1 ) {
				$this->add_render_attribute( 'wrapper', 'data-slider-settings', wp_json_encode([
					...
					'speed' => 700,
					'animateHeight' => $this->_settings['stretch_image'] !== '',
				]) );
			}
		...
		public function render_caption( $slide, $key )
		{
			if( $slide['captions'] !== '' ):
				...
				if( $this->_settings['stretch_image'] !== '' ){
		...
		public function render_arrows()
		{
			if( $this->_settings['arrows'] === 'yes' ):
				...
				<div class="cUno-arrows <?php echo implode(' ', $classes) ?>">
					<?php
						echo reycore__arrowSvg(false, false, 'data-dir="-1"');
						echo reycore__arrowSvg(true, false, 'data-dir="+1"');
					?>
				</div>
			<?php endif;
		}
		...
		protected function render() {
			...
			reyCoreAssets()->add_styles(['reycore-widget-carousel-uno-styles', 'rey-splide']);
