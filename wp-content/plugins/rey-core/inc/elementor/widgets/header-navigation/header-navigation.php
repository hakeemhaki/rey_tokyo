...
class ReyCore_Widget_Header_Navigation extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						...
						'{{WRAPPER}}' => '--indicator-padding: {{RIGHT}}{{UNIT}};',
						'.rtl {{WRAPPER}}' => '--indicator-padding: {{LEFT}}{{UNIT}};',
					],
				]
			);
		...
		$this->add_control(
			'submenus_padding_y',
			[
				'label' => esc_html__( 'Submenus Padding (Vertical)', 'rey-core' ) . ' (px)',
				'type' => \Elementor\Controls_Manager::NUMBER,
				'default' => '',
				'min' => 0,
				'max' => 1000,
		...
		$this->add_control(
			'submenus_padding_x',
			[
				'label' => esc_html__( 'Submenus Padding (Horizontal)', 'rey-core' ) . ' (px)',
				'type' => \Elementor\Controls_Manager::NUMBER,
				'default' => '',
				'min' => 0,
				'max' => 1000,
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Submenus Display', 'rey-core' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'collapsed',
				'options' => [
		...
		$this->add_control(
			...
			[
			   'label' => esc_html__( 'Logo Max-Width', 'rey-core' ) . ' (px)',
				...
				'selectors' => [
					'{{WRAPPER}} .rey-mobileNav-header .rey-siteLogo img, {{WRAPPER}} .rey-mobileNav-header .rey-siteLogo .custom-logo' => 'max-width: {{SIZE}}px;',
				],
				'separator' => 'before'
			]
		);
	...
	function set_nav_options( $args ){
		...
		if( $settings['custom'] ){
			...
			if ($settings['mobile_menu_source_type'] === 'offcanvas' && ($gs_offcanvas = $settings['offcanvas_panel'])){
				$args['load_hamburger'] = [
					'attributes' => [
		...
		if( $settings['offcanvas_tap_behavior'] === 'link' ){
			if( in_array($settings['nav_indicator'], ['circle', 'dash']) ){
				// TODO find better patch
				// $settings['nav_indicator'] = 'plus';
			}
		}
	...
	protected function render() {
		...
		// Social Icons
		add_action('rey/mobile_nav/footer', [$this, 'render_social'], 0);
		...
		add_filter('wp_nav_menu_args', [$this, 'set_mega_menu_support'], 9);
			...
			if( $settings['custom'] && $settings['enable_mobile'] !== '' && $settings['breakpoint'] ){
		...
		remove_filter('reycore/menu_nav/support_icons', '__return_false');
		remove_action('rey/mobile_nav/footer', [$this, 'render_social'], 0);
		...
		if ($settings['custom'] && $settings['mobile_menu_source_type'] === 'offcanvas' && ($gs_offcanvas = $settings['offcanvas_panel'])){
