...
if( class_exists('WooCommerce') && !class_exists('ReyCore_Widget_Wc_Cart') ):
 ...
 * @since 1.0.0
 */
class ReyCore_Widget_Wc_Cart extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'reycore-wc-cart';
	}

	public function get_title() {
		return __( 'WooCommerce Cart Page', 'rey-core' );
	}
			...
			$this->add_control(
				...
				[
					...
					'raw' => sprintf(
						...
						add_query_arg( ['autofocus[section]' => 'woocommerce_cart_rey'], admin_url( 'customize.php' ) ) ),
	...
	public function display_custom_layout(){
		...
		if ( WC()->cart->is_empty() ) {
			...
			// prepare
			add_action( 'woocommerce_after_cart_form_wrapper', [ReyCore_WooCommerce_Loop::getInstance(), 'set_loop_props'], 0);
			add_action( 'woocommerce_after_cart_form_wrapper', [ReyCore_WooCommerce_Loop::getInstance(), 'components_add_remove'], 5);
