...
if( !class_exists('ReyCore_Gutenberg') ):
/**
 * Credits @Coblocks
 * https://github.com/godaddy-wordpress/coblocks/blob/master/includes/class-coblocks-block-assets.php
 *
 */
class ReyCore_Gutenberg
{
	...
	function acf_block__postsv1( $block, $content = '', $is_preview = false, $post_id = 0){
		...
		// error_log(var_export( [$args, $query_args],1));
	...
	function acf_block__elementor_global_section( $block, $content = '', $is_preview = false, $post_id = 0){
		...
		if( class_exists('ReyCore_GlobalSections') && isset($block['data']['global_section']) && $gs = $block['data']['global_section'] ){
	...
	function register_acf_block_scripts()
	{
		reyCoreAssets()->register_asset('styles', $this->blocks_styles());
		reyCoreAssets()->register_asset('scripts', $this->blocks_scripts());
	}
