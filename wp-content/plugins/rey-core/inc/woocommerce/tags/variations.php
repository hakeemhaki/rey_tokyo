...
class ReyCore_WooCommerce_Variations
{
	...
	private function __construct()
	{
		...
		// TODO CHECK PRO SETTINGS
		add_action('reycore/woocommerce/loop/scripts', function(){
			reyCoreAssets()->add_styles('rey-plugin-wvs');
		});

		add_action('reycore/woocommerce/product_page/scripts', function(){
			reyCoreAssets()->add_styles('rey-plugin-wvs');
		});
	...
	static function load_scripts(){
		reyCoreAssets()->add_styles('rey-wc-tag-variations');
		reyCoreAssets()->add_scripts('reycore-wc-loop-variations');
	}
	...
	function get_variation_data( $product, $attr_data )
	{
		...
		if( false === ($data = get_transient( self::TRANSIENT_PRODUCT_VARIATIONS . $prod_id )) ){
			...
			if( $this->store_transients ){
				set_transient( self::TRANSIENT_PRODUCT_VARIATIONS . $prod_id, $variation_data, WEEK_IN_SECONDS );
			}
	...
	function get_regular_attribute_data( $product, $attr_data ){
		...
		if( isset( $all_attributes[$attr_data['taxonomy_name']] ) && $attribute = $all_attributes[$attr_data['taxonomy_name']] ){
			...
			$attribute_name = is_object($attribute) ? $attribute->get_name() : $attribute;
	...
	public static function get_attributes_list( $wc_taxonomies = false ){
	...
	function clean_db_transients(){
		...
		$like_main = '%transient_' . $wpdb->esc_like( self::TRANSIENT_PRODUCT_VARIATIONS ) . '%';
		$like_timeout = '%transient_timeout_' . $wpdb->esc_like( self::TRANSIENT_PRODUCT_VARIATIONS ) . '%';
