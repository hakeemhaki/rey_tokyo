...
if( class_exists('ReyTheme_API') && ! class_exists('ReyTheme_Rollback') ):
	...
	class ReyTheme_Rollback extends ReyTheme_API
	{
		...
		function ajax_rollback(){
			...
			if( isset($_REQUEST['key']) && ($key = $this->clean($_REQUEST['key'])) && isset($this->items_data[$key]) ){
				...
				if( isset($_REQUEST['version']) && ($version = $this->clean($_REQUEST['version'])) && version_compare( $version, '0.0.1', '>=' ) ){
					...
					$item = [
						'url' => $this->get_download_url($key, $version),
						'version' => $version,
						'type' => 'plugin',
					...
					if( $key === 'theme' ){
						$item['type'] = 'theme';
						$item['basename'] = $this->items_data[$key]['slug'];
					}

					$this->apply_package( $item );
					$this->upgrade( $item );
		 ...
		 * @since 2.0.0
		 */
		private function apply_package( $item ) {

			$update_transient = get_site_transient( "update_{$item['type']}s" );
			...
			if ( 'theme' === $item['type'] ) {
				$item_info                = [];
				$item_info['new_version'] = $item['version'];
				$item_info['plugin']      = $item['basename'];
				$item_info['slug']        = $item['slug'];
				$item_info['package']     = $item['url'];
			} else {
			...
			$update_transient->response[ $item['basename'] ] = $item_info;

			remove_all_filters( "pre_set_site_transient_update_{$item['type']}s" );
			set_site_transient( "update_{$item['type']}s", $update_transient );
		}
		 ...
		 * @since 2.0.0
		 */
		private function upgrade( $item ) {

			require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

			$skin     = new WP_Ajax_Upgrader_Skin();
			$upgrader = ucfirst( $item['type'] ) . '_Upgrader';
			$upgrader = new $upgrader( $skin );
			$result   = $upgrader->bulk_upgrade( [ $item['basename'] ] );
		...
		public function get_html( $key ){
			...
			foreach($versions as $version){
				...
				if( $key === 'theme' ){
					...
					if( version_compare( $this->items_data['rey-core']['current_version'], self::THRESHOLD_VERSION, '>=' ) &&
						...
						$message = sprintf(esc_html_x('To downgrade Rey Theme to versions lower than %s, Rey-Core must be downgraded first.', 'Backend dashboard text', 'rey'), self::THRESHOLD_VERSION);
			...
			printf(
				'<label class="__rb-title">%4$s</label><select>%1$s</select><button class="js-dashRollback button disabled" data-settings=\'%3$s\'>%2$s</button>',
				$options,
		...
		public function get_versions( $data, $key ){
			...
			$get_versions = $this->file_remote_get( self::API_FILES_URL . $data['file'] );
